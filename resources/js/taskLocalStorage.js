...
export default class Task {
    ...
    static #create = async (table) => {
        ...
        const data = Task.#formData // recuperar los datos mediante el accesor
        data['id'] = Task.#random() // agregar un id aleatorio al objeto de datos
        //data.activity = data.activity.remover(['<','>','/','div','<em>','</em>','&nbsp','br', 'strong','<u>','</u>', 'span','<ol>', '</ol>', '<li>','</li>'])


        table.addRow(data, true); // agrega los datos al inicio de la tabla. Ver esto
        localStorage.setItem('tasks', JSON.stringify(table.getData())) /* agrega los datos en el localStorage (34)*/
        Task.#modal.dispose() // elimina el formulario de entrada de datos
        Toast.info({ message: 'Registro agregado' })
    }
    ...
    static #update = async (row) => {
        ...
        row.update(data) // Ver updating data
        localStorage.setItem('tasks', JSON.stringify(row.getTable().getData())) /* guarda los datos en el localStorage (34)*/
        Task.#modal.dispose()
        Toast.info({ message: 'Registro actualizado' })
    }
    ...
    static delete = async (row) => {
        //console.log(row)
        row.delete()
        localStorage.setItem('tasks', JSON.stringify(row.getTable().getData()))/* Guarda los datos en localStorage (34) */
        Toast.info({ message: 'Registro eliminado' })
